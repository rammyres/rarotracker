...
def main():
    ...
    print("\n--- Telegram (opcional, gratuito) ---")
    if ask_yes_no("Configurar alertas via Telegram?", default_yes=True):
        print(
            "  1. No Telegram, fale com @BotFather, envie /newbot e siga as instruções.\n"
            "  2. Copie o token que ele te der.\n"
            "  3. Fale com @userinfobot (ou mande qualquer mensagem pro seu bot novo e\n"
            "     depois abra https://api.telegram.org/bot<TOKEN>/getUpdates no navegador\n"
            "     pra achar seu chat_id)."
        )
        env["TELEGRAM_BOT_TOKEN"] = ask("TELEGRAM_BOT_TOKEN")
        env["TELEGRAM_CHAT_ID"] = ask("TELEGRAM_CHAT_ID")
    else:
